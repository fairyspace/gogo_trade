...
CREATE TABLE `t_partner_program` (
  ...
  `program_code` VARCHAR(64) NOT NULL COMMENT 'Program编码',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_name` VARCHAR(128) NOT NULL COMMENT 'Program名称',
  ...
  `max_hierarchy_depth` TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '允许的最大层级深度，建议1-3，禁止默认无限层级',
  `attribution_window_days` INT UNSIGNED NOT NULL DEFAULT 30 COMMENT '归因窗口天数',
  `cooling_off_days` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '冷静期天数',
  `settlement_cycle` VARCHAR(16) NOT NULL DEFAULT 'MONTHLY' COMMENT '结算周期：MONTHLY/WEEKLY等',
  `hold_days` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '保留金保留天数',
  `min_payout_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '最低出款金额',
  `settlement_currency` VARCHAR(8) NOT NULL COMMENT 'Program结算币种，同Program下所有Agreement强制一致',
  `allowed_countries_json` JSON DEFAULT NULL COMMENT '允许招揽的辖区国家码列表(JSON数组)',
  `blocked_countries_json` JSON DEFAULT NULL COMMENT '禁止招揽的辖区国家码列表(JSON数组)',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT 'Program状态：DRAFT/ACTIVE/SUSPENDED/CLOSED',
...
CREATE TABLE `t_partner` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `partner_no` VARCHAR(64) NOT NULL COMMENT 'Partner对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `user_id` BIGINT UNSIGNED NOT NULL COMMENT '对应t_user.id',
  `partner_type` VARCHAR(32) NOT NULL COMMENT 'IB/AFFILIATE/INSTITUTIONAL/HYBRID',
  `display_name` VARCHAR(128) NOT NULL COMMENT 'Partner对外展示名称',
  `company_name` VARCHAR(256) DEFAULT NULL COMMENT '机构名称，个人Partner可为空',
  `country_code` CHAR(2) NOT NULL COMMENT '国家/地区码(ISO 3166-1 alpha-2)',
  `kyb_application_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联Onboarding域KYB申请单ID',
  `risk_level` VARCHAR(16) NOT NULL DEFAULT 'NORMAL' COMMENT '风险等级：NORMAL/ELEVATED/HIGH',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT 'Partner状态机：DRAFT/PENDING_KYB/PENDING_AGREEMENT/PENDING_APPROVAL/ACTIVE/SUSPENDED/TERMINATING/TERMINATED/REJECTED（见三、3.2）',
  `version` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
  `suspended_reason` VARCHAR(512) DEFAULT NULL COMMENT '暂停原因',
  `activated_at` DATETIME(3) DEFAULT NULL COMMENT '首次激活(ACTIVE)时间',
  `terminated_at` DATETIME(3) DEFAULT NULL COMMENT '终止(TERMINATED)时间',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_hierarchy` (
  ...
  `hierarchy_version` BIGINT UNSIGNED NOT NULL COMMENT 'Program级单调递增层级版本号，每次发布生成全树邻接边与Closure快照',
  `effective_from` DATETIME(3) NOT NULL,
  `effective_to` DATETIME(3) DEFAULT NULL,
  `change_reason` VARCHAR(512) NOT NULL COMMENT '层级变更原因',
  `approval_id` VARCHAR(64) NOT NULL COMMENT '层级变更审批单号',
...
CREATE TABLE `t_partner_hierarchy_version_current` (
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `hierarchy_version` BIGINT UNSIGNED NOT NULL COMMENT '此刻生效的全树层级版本号',
  `effective_from` DATETIME(3) NOT NULL,
  `version` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
...
CREATE TABLE `t_partner_hierarchy_closure` (
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `hierarchy_version` BIGINT UNSIGNED NOT NULL COMMENT '层级版本号',
  `ancestor_partner_id` BIGINT UNSIGNED NOT NULL COMMENT '祖先Partner ID',
  `descendant_partner_id` BIGINT UNSIGNED NOT NULL COMMENT '后代Partner ID',
  `depth` TINYINT UNSIGNED NOT NULL COMMENT '祖先到后代的层级深度，直属为1',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `t_partner_agreement` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `agreement_no` VARCHAR(64) NOT NULL COMMENT 'Agreement对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `partner_id` BIGINT UNSIGNED NOT NULL,
  `agreement_version` INT UNSIGNED NOT NULL COMMENT 'Agreement版本号，同一Partner+Program下单调递增，已发布版本不可原地修改',
  `commission_plan_version_id` BIGINT UNSIGNED NOT NULL COMMENT '关联t_commission_plan_version.id',
  `settlement_currency` VARCHAR(8) NOT NULL COMMENT '结算币种，须与所属Program.settlement_currency一致',
  `tax_profile_json` JSON DEFAULT NULL COMMENT '协议内嵌税务信息快照',
  `effective_from` DATETIME(3) NOT NULL,
  `effective_to` DATETIME(3) DEFAULT NULL,
  `signed_evidence_id` VARCHAR(128) DEFAULT NULL COMMENT '签署证据ID(Onboarding侧存证)',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT 'Agreement状态：DRAFT/PENDING_APPROVAL/ACTIVE/EXPIRED/TERMINATED',
  `version` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
...
CREATE TABLE `t_partner_campaign` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `campaign_code` VARCHAR(64) NOT NULL COMMENT '推广活动编码',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `partner_id` BIGINT UNSIGNED NOT NULL,
  `campaign_name` VARCHAR(128) NOT NULL COMMENT '推广活动名称',
  `referral_code` VARCHAR(64) NOT NULL COMMENT '推荐码，用于解析campaign_id+partner_id并生成Attribution',
  `landing_url` VARCHAR(512) DEFAULT NULL COMMENT '落地页URL',
  `allowed_countries_json` JSON DEFAULT NULL COMMENT '允许投放的国家码列表(JSON数组)',
  `effective_from` DATETIME(3) NOT NULL,
  `effective_to` DATETIME(3) DEFAULT NULL,
  `status` VARCHAR(24) NOT NULL DEFAULT 'ACTIVE' COMMENT '活动状态：ACTIVE/PAUSED/CLOSED',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_attribution` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `attribution_no` VARCHAR(64) NOT NULL COMMENT '归因记录对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `partner_id` BIGINT UNSIGNED NOT NULL,
  `campaign_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联t_partner_campaign.id，可为空（如ADMIN迁移无campaign）',
  `subject_type` VARCHAR(16) NOT NULL COMMENT 'USER/ACCOUNT',
  `subject_id` BIGINT UNSIGNED NOT NULL COMMENT '被归因主体ID，对应subject_type所指user_id或account_id',
  `source_type` VARCHAR(24) NOT NULL COMMENT 'TRACKING/REFERRAL/ADMIN/INHERITED/MIGRATION',
  `tracking_token_hash` CHAR(64) DEFAULT NULL COMMENT 'Tracking Token哈希值，用于设备匹配点击证据',
  `evidence_json` JSON DEFAULT NULL COMMENT '归因证据快照(JSON)',
  `effective_from` DATETIME(3) NOT NULL,
  `effective_to` DATETIME(3) DEFAULT NULL,
  `change_reason` VARCHAR(512) DEFAULT NULL COMMENT '归因变更/迁移原因',
  `approval_id` VARCHAR(64) DEFAULT NULL COMMENT '管理员迁移审批单号',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_attribution_current` (
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `subject_type` VARCHAR(16) NOT NULL COMMENT 'USER/ACCOUNT',
  `subject_id` BIGINT UNSIGNED NOT NULL COMMENT '被归因主体ID',
  `attribution_id` BIGINT UNSIGNED NOT NULL COMMENT '指向此刻生效的t_partner_attribution.id',
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
...
CREATE TABLE `t_commission_plan_version` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `plan_code` VARCHAR(64) NOT NULL COMMENT '佣金计划编码',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `version` INT UNSIGNED NOT NULL COMMENT '计划版本号，同entity_id+plan_code下单调递增，已发布版本不可UPDATE',
  `allocation_mode` VARCHAR(32) NOT NULL COMMENT 'CUMULATIVE_DELTA/FIXED_OVERRIDE，同一Plan Version只能选择其一',
  `plan_cap_type` VARCHAR(16) NOT NULL DEFAULT 'NONE' COMMENT 'NONE/RATIO/AMOUNT',
  `plan_cap_value` DECIMAL(24,10) DEFAULT NULL COMMENT '上限值，RATIO时为比例、AMOUNT时为金额',
  ...
  `effective_from` DATETIME(3) NOT NULL,
  `effective_to` DATETIME(3) DEFAULT NULL,
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT '计划状态：DRAFT/PUBLISHED/EXPIRED',
...
CREATE TABLE `t_commission_budget_pool` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `pool_no` VARCHAR(64) NOT NULL COMMENT '预算池对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  ...
  `limit_amount` DECIMAL(24,8) NOT NULL COMMENT '本周期预算上限金额',
  `reserved_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '已预占但未最终消耗的金额',
  `consumed_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '已实际消耗的金额',
  `version` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
  `status` VARCHAR(16) NOT NULL DEFAULT 'ACTIVE' COMMENT '预算池状态：ACTIVE/EXHAUSTED/CLOSED',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_commission_rule` (
  ...
  `beneficiary_level` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '受益层级，0为直属Partner，向上递增',
  `reward_type` VARCHAR(32) NOT NULL COMMENT 'PER_LOT/PER_DEAL/NOTIONAL_BPS/COMMISSION_SHARE/NET_REVENUE_SHARE/QUALIFIED_ACCOUNT_CPA/FIRST_DEPOSIT_CPA/FIXED_OVERRIDE（见五、5.1）',
  `reward_value` DECIMAL(24,10) NOT NULL COMMENT '奖励数值，含义随reward_type而定（单价/比例/固定额）',
  `reward_currency` VARCHAR(8) DEFAULT NULL COMMENT '固定金额类奖励的计价币种(Rule Currency)',
  `account_group_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'Scope匹配：账户组ID',
  `symbol_category_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'Scope匹配：品种大类ID',
  `symbol_group_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'Scope匹配：品种分组ID',
  `symbol_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'Scope匹配：具体品种ID，优先级最高',
  `entry_scope` VARCHAR(16) NOT NULL DEFAULT 'BOTH' COMMENT 'IN/OUT/BOTH',
  `qualification_json` JSON DEFAULT NULL COMMENT '合格量资格条件快照(JSON)，如KYC/最小净入金/保留天数等',
  `min_reward` DECIMAL(24,8) DEFAULT NULL COMMENT '单笔奖励最小值',
  `max_reward` DECIMAL(24,8) DEFAULT NULL COMMENT '单笔奖励最大值',
  `priority` INT NOT NULL DEFAULT 0 COMMENT '同优先级Scope的决胜优先级，越大越先匹配',
  `status` VARCHAR(16) NOT NULL DEFAULT 'ACTIVE' COMMENT '规则状态：ACTIVE/INACTIVE，发布后不可UPDATE，仅可创建新版本',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_commission_tier` (
  ...
  `tier_no` SMALLINT UNSIGNED NOT NULL COMMENT '阶梯序号，从小到大排列',
  ...
  `threshold_from` DECIMAL(24,8) NOT NULL COMMENT '本档区间起点（含）',
  `threshold_to` DECIMAL(24,8) DEFAULT NULL COMMENT '本档区间终点（不含），为空表示无上限',
  `reward_value` DECIMAL(24,10) NOT NULL COMMENT '本档奖励数值',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_event_inbox` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `event_id` VARCHAR(64) NOT NULL COMMENT '生产方事件ID',
  `event_type` VARCHAR(64) NOT NULL COMMENT '事件类型，如deal.created.v1',
  `producer` VARCHAR(64) NOT NULL COMMENT '事件生产方标识',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `aggregate_id` VARCHAR(64) NOT NULL COMMENT '事件聚合根ID',
  `aggregate_version` BIGINT UNSIGNED DEFAULT NULL COMMENT '聚合根版本号',
  `occurred_at` DATETIME(3) NOT NULL,
  `payload_json` JSON NOT NULL COMMENT '事件原始payload',
  `payload_hash` CHAR(64) NOT NULL COMMENT 'payload哈希值，用于幂等/完整性校验',
  `status` VARCHAR(24) NOT NULL DEFAULT 'RECEIVED' COMMENT '处理状态：RECEIVED/PROCESSING/PROCESSED/FAILED',
  `retry_count` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '重试次数',
  `next_retry_at` DATETIME(3) DEFAULT NULL,
  `error_message` VARCHAR(1024) DEFAULT NULL COMMENT '最近一次处理失败的错误信息',
...
CREATE TABLE `t_commission_accrual` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `accrual_no` VARCHAR(64) NOT NULL COMMENT '计提头对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `plan_version_id` BIGINT UNSIGNED NOT NULL,
  `source_producer` VARCHAR(64) NOT NULL COMMENT '触发事件的生产方标识',
  `source_event_id` VARCHAR(64) NOT NULL COMMENT '触发事件ID（幂等键组成部分）',
  `source_event_type` VARCHAR(64) NOT NULL COMMENT '触发事件类型，如deal.created.v1/deposit.completed.v1',
  `source_biz_id` VARCHAR(64) NOT NULL COMMENT '触发事件对应的业务单据ID，如deal_id',
  `client_user_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '被归因客户user_id，可为空',
  `client_account_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '被归因客户交易账户ID，可为空',
  `attribution_id` BIGINT UNSIGNED NOT NULL,
  `hierarchy_version` BIGINT UNSIGNED NOT NULL COMMENT '计提时使用的层级版本号快照',
  `hierarchy_snapshot_json` JSON NOT NULL COMMENT '计提时归因链层级快照(JSON)，用于历史重算与迁移隔离',
  `calculation_input_json` JSON NOT NULL COMMENT '计算输入快照(事件、规则、汇率等)',
  `calculation_explain_json` JSON NOT NULL COMMENT '计算过程可解释明细，用于100%重放',
  `qualified_quantity` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '合格数量(qualified_volume)',
  `distributable_pool_amount` DECIMAL(24,8) DEFAULT NULL COMMENT '本次可分配池金额',
  `pool_currency` VARCHAR(8) DEFAULT NULL COMMENT '可分配池币种',
  `total_reward_amount` DECIMAL(24,8) NOT NULL COMMENT '本次计提总奖励金额，等于全部Allocation之和',
  `reward_currency` VARCHAR(8) NOT NULL COMMENT '奖励结算币种',
  `calculation_run_id` VARCHAR(64) NOT NULL COMMENT '计算运行批次ID，用于补算追溯',
  `status` VARCHAR(24) NOT NULL DEFAULT 'CALCULATED' COMMENT '计提状态：CALCULATED/CALCULATION_ERROR/BUDGET_EXCEEDED',
  `business_date` DATE NOT NULL COMMENT '业务日期(按Program时区)',
  `occurred_at` DATETIME(3) NOT NULL,
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_commission_allocation` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `allocation_no` VARCHAR(64) NOT NULL COMMENT '分配记录对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `accrual_id` BIGINT UNSIGNED NOT NULL,
  `beneficiary_partner_id` BIGINT UNSIGNED NOT NULL COMMENT '受益Partner ID',
  `beneficiary_level` TINYINT UNSIGNED NOT NULL COMMENT '受益层级，0为直属Partner，向上递增',
  `rule_id` BIGINT UNSIGNED NOT NULL,
  `agreement_id` BIGINT UNSIGNED NOT NULL,
  `source_base_amount` DECIMAL(24,8) NOT NULL COMMENT '转换前原始基数金额',
  `source_currency` VARCHAR(8) NOT NULL COMMENT '转换前原始币种',
  `rate` DECIMAL(24,10) NOT NULL COMMENT '本级适用的累计权益费率/比例',
  `gross_amount` DECIMAL(24,8) NOT NULL COMMENT '税前应付金额(Settlement币种)',
  `tax_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '预扣税金额',
  `hold_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '冻结/保留金金额',
  `net_amount` DECIMAL(24,8) NOT NULL COMMENT '净应付金额=gross_amount-tax_amount',
  `currency` VARCHAR(8) NOT NULL COMMENT 'Settlement结算币种',
  `fx_rate` DECIMAL(24,12) DEFAULT NULL COMMENT '原始币种转结算币种汇率',
  `fx_source` VARCHAR(64) DEFAULT NULL COMMENT '汇率来源',
  `eligible_at` DATETIME(3) DEFAULT NULL,
  `settlement_item_id` BIGINT UNSIGNED DEFAULT NULL,
  `reversal_of_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '被冲正的原Allocation ID，冲正记录专用',
  `status` VARCHAR(24) NOT NULL DEFAULT 'PENDING' COMMENT '生命周期：PENDING/ELIGIBLE/HELD/PAYABLE/PAID/REVERSED/REJECTED（见六、6.2）',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_commission_adjustment` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `adjustment_no` VARCHAR(64) NOT NULL COMMENT '调整记录对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `partner_id` BIGINT UNSIGNED NOT NULL,
  `original_allocation_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '原始Allocation ID，可为空（如纯人工调账）',
  ...
  `currency` VARCHAR(8) NOT NULL,
  `reason_code` VARCHAR(64) NOT NULL COMMENT '调整原因码',
  `reason_detail` VARCHAR(1024) NOT NULL COMMENT '调整原因详情',
  `status` VARCHAR(24) NOT NULL DEFAULT 'PENDING_APPROVAL' COMMENT '状态：PENDING_APPROVAL/APPROVED/REJECTED',
  `version` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
  `created_by` BIGINT UNSIGNED NOT NULL COMMENT '创建人ID(Maker)',
  `approved_by` BIGINT UNSIGNED DEFAULT NULL COMMENT '审批人ID(Checker)',
  `approved_at` DATETIME(3) DEFAULT NULL,
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_ledger_account` (
  ...
  `account_code` VARCHAR(64) NOT NULL COMMENT '科目编码，如PARTNER_PAYABLE:{partner_id}/PAYOUT_CLEARING/COMMISSION_EXPENSE',
  `account_type` VARCHAR(24) NOT NULL COMMENT 'EXPENSE/PAYABLE/AVAILABLE/HELD/CLEARING/CASH/ROUNDING',
  `currency` VARCHAR(8) NOT NULL,
  `status` VARCHAR(16) NOT NULL DEFAULT 'ACTIVE' COMMENT '科目状态：ACTIVE/CLOSED',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_journal` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `journal_no` VARCHAR(64) NOT NULL COMMENT '凭证对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  ...
  `business_id` VARCHAR(64) NOT NULL COMMENT '业务单据ID，与business_type对应',
  `business_type` VARCHAR(32) NOT NULL COMMENT '业务单据类型，如ACCRUAL/ALLOCATION/PAYOUT等',
  `currency` VARCHAR(8) NOT NULL,
  `total_debit` DECIMAL(24,8) NOT NULL COMMENT '借方合计，须等于total_credit',
  `total_credit` DECIMAL(24,8) NOT NULL COMMENT '贷方合计，须等于total_debit',
  `reversal_of_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '被冲销的原Journal ID，冲正记录专用',
  `description` VARCHAR(512) DEFAULT NULL COMMENT '凭证说明',
...
CREATE TABLE `t_partner_journal_entry` (
  ...
  `line_no` SMALLINT UNSIGNED NOT NULL COMMENT '凭证内分录行号',
  `ledger_account_id` BIGINT UNSIGNED NOT NULL,
  `direction` VARCHAR(8) NOT NULL COMMENT 'DEBIT/CREDIT',
  `amount` DECIMAL(24,8) NOT NULL COMMENT '分录绝对值金额',
  `currency` VARCHAR(8) NOT NULL,
  `partner_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联Partner ID，平台科目分录可为空',
  `allocation_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联的Allocation ID，可为空',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_settlement_claim` (
  ...
  `source_id` BIGINT UNSIGNED NOT NULL COMMENT '被认领来源ID，对应source_type所指Allocation或Adjustment',
  `claim_token` VARCHAR(64) NOT NULL COMMENT '认领令牌，用于幂等校验',
...
CREATE TABLE `t_partner_settlement` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `settlement_no` VARCHAR(64) NOT NULL COMMENT '结算单对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `program_id` BIGINT UNSIGNED NOT NULL,
  `partner_id` BIGINT UNSIGNED NOT NULL,
  `period_start` DATETIME(3) NOT NULL,
  `period_end` DATETIME(3) NOT NULL,
  `currency` VARCHAR(8) NOT NULL,
  `opening_balance` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '期初余额',
  `gross_amount` DECIMAL(24,8) NOT NULL COMMENT '本期计提税前应付合计',
  `adjustment_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '本期调整/回拨合计（有符号）',
  `tax_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '本期预扣税合计',
  `hold_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '本期冻结/保留金合计',
  `net_payable_amount` DECIMAL(24,8) NOT NULL COMMENT '本期净应付金额',
  `reserved_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '已被Payout Application预占的金额',
  `paid_amount` DECIMAL(24,8) NOT NULL DEFAULT 0 COMMENT '已实际出款金额',
  `closing_balance` DECIMAL(24,8) NOT NULL COMMENT '期末余额',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT '状态机：DRAFT/CALCULATED/PENDING_APPROVAL/APPROVED/REJECTED/POSTED/PARTIALLY_PAID/PAID（见六、6.5）',
  `version` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
  `maker_id` BIGINT UNSIGNED NOT NULL COMMENT '结算发起人(Maker)',
  `checker_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '结算审批人(Checker)，与maker_id不得相同',
  `approved_at` DATETIME(3) DEFAULT NULL,
...
CREATE TABLE `t_partner_settlement_item` (
  ...
  `allocation_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'item_type=ALLOCATION时必填，全局唯一，防止重复进入结算单',
  `adjustment_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'item_type=ADJUSTMENT时必填，全局唯一，防止重复进入结算单',
  `source_ref` VARCHAR(64) DEFAULT NULL COMMENT 'TAX/HOLD/RELEASE等非佣金来源业务键',
  `amount` DECIMAL(24,8) NOT NULL COMMENT '本明细金额（有符号）',
  `currency` VARCHAR(8) NOT NULL,
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_payout` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `payout_no` VARCHAR(64) NOT NULL COMMENT '出款单对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `partner_id` BIGINT UNSIGNED NOT NULL,
  `amount` DECIMAL(24,8) NOT NULL COMMENT '出款金额',
  `currency` VARCHAR(8) NOT NULL,
  `payout_type` VARCHAR(24) NOT NULL COMMENT 'EXTERNAL/TRANSFER_TO_TRADE',
  `payment_method_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'EXTERNAL出款时使用的Fund支付方式ID',
  `target_trading_account_id` BIGINT UNSIGNED DEFAULT NULL COMMENT 'TRANSFER_TO_TRADE时的目标交易账户ID',
  ...
  `idempotency_key` VARCHAR(64) NOT NULL COMMENT '幂等键，范围(entity_id,idempotency_key)',
  `risk_snapshot_json` JSON NOT NULL COMMENT '发起出款时的风险/资格校验快照',
  ...
  `version` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁',
  `requested_by` BIGINT UNSIGNED NOT NULL COMMENT '出款申请人',
  `approved_by` BIGINT UNSIGNED DEFAULT NULL COMMENT '出款审批人，与requested_by角色互斥（PMS_IB_PAYOUT_APPROVER）',
  `submitted_at` DATETIME(3) DEFAULT NULL,
  `completed_at` DATETIME(3) DEFAULT NULL,
  `failure_reason` VARCHAR(512) DEFAULT NULL COMMENT '出款失败原因',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_payout_application` (
  ...
  `settlement_id` BIGINT UNSIGNED NOT NULL,
  `sequence_no` SMALLINT UNSIGNED NOT NULL COMMENT '同一Payout下核销顺序号',
  `applied_amount` DECIMAL(24,8) NOT NULL COMMENT '本次核销金额',
  `currency` VARCHAR(8) NOT NULL,
...
CREATE TABLE `t_partner_audit_log` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `audit_no` VARCHAR(64) NOT NULL COMMENT '审计记录对外编号',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `actor_type` VARCHAR(16) NOT NULL COMMENT 'STAFF/PARTNER/SYSTEM',
  `actor_id` BIGINT UNSIGNED NOT NULL COMMENT '操作人ID，含义随actor_type而定',
  `action` VARCHAR(64) NOT NULL COMMENT '操作动作，如CREATE/APPROVE/SUSPEND/TRANSFER等',
  `resource_type` VARCHAR(64) NOT NULL COMMENT '被操作资源类型，如t_partner/t_partner_agreement等',
  `resource_id` VARCHAR(64) NOT NULL COMMENT '被操作资源ID',
  ...
  `correlation_id` VARCHAR(64) DEFAULT NULL COMMENT '跨系统关联追踪ID',
  `ip_address` VARBINARY(16) DEFAULT NULL COMMENT '操作来源IP',
  `signature` CHAR(64) NOT NULL COMMENT '记录防篡改签名',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_partner_outbox_event` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `event_id` VARCHAR(64) NOT NULL COMMENT '本域发布的事件ID',
  `entity_id` BIGINT UNSIGNED NOT NULL,
  `event_type` VARCHAR(64) NOT NULL COMMENT '事件类型，如partner.payout.completed.v1等',
  `aggregate_type` VARCHAR(64) NOT NULL COMMENT '聚合根类型',
  `aggregate_id` VARCHAR(64) NOT NULL COMMENT '聚合根ID',
  `aggregate_version` BIGINT UNSIGNED NOT NULL COMMENT '聚合根版本号',
  `payload_json` JSON NOT NULL COMMENT '事件payload',
  `status` VARCHAR(16) NOT NULL DEFAULT 'NEW' COMMENT '发布状态：NEW/PUBLISHED/FAILED',
  `retry_count` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '重试次数',
  `next_retry_at` DATETIME(3) DEFAULT NULL,
  `published_at` DATETIME(3) DEFAULT NULL,
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
CREATE TABLE `t_partner_tax_profile` (
  ...
  `effective_from` DATETIME(3) NOT NULL,
  `effective_to` DATETIME(3) DEFAULT NULL,
  `status` VARCHAR(16) NOT NULL DEFAULT 'ACTIVE' COMMENT '档案状态：ACTIVE/INACTIVE',
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE t_partner_channel_click_log (
  ...
  `converted_subject_id`   Nullable(Int64) COMMENT '转化后回填的主体ID，对应converted_subject_type',
