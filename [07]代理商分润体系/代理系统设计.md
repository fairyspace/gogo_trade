...
| `partner.payout.failed.v1` | Fund | 释放或继续冻结出款金额（域内事件，与《资金系统设计》§3.3 一致，尚未登记平台事件表，跨域订阅前需补充登记） |
...
- `customer_commission_received_amount` 在事件中应为非负标准化金额；现状 `t_deal.commission` 字段在交易域内部仍以“负值表示扣费”存储（见 [10]交易系统/交易系统设计.md §9.4），事件适配层（消费 `deal.created.v1` 时）必须显式取绝对值后再进入计提计算，不得直接代入公式。
...
5. Fund 回调 `partner.payout.failed.v1`（域内事件，尚未登记平台事件表，见二、2.2）：`PAYOUT_RELEASE` Journal 将 Clearing → Available，Payout=FAILED；
6. 超时保持 PROCESSING，由对账任务查询 `t_payout_instruction.status`，禁止直接重复出款。

**Fund 回调超时处理**：

```text
Payout 发起 → t_payout_instruction(status=SUBMITTED)
  → PROCESSING（等待 Fund 回调）
  → 超时 24h 无响应：
      1. t_partner_payout.status → RECONCILE
      2. 告警 P1
      3. 人工核对 t_payout_instruction 实际状态
      4. Fund 确认已出款(SUCCEEDED) → COMPLETE
      5. Fund 确认未出款(FAILED/CANCELLED) → RELEASE（释放锁定金额）
  → Fund 回调成功 → COMPLETE
  → Fund 回调失败 → RELEASE
```
