# 全球多资产代理商系统设计（Partner / IB Service）v2.0
...
| `account.activated.v1` | Account | 更新转化里程碑 |
...
| `partner.payout.completed.v1` | Fund | 完成出款账务 |
| `partner.payout.failed.v1` | Fund | 释放或继续冻结出款金额 |
...
当前成交与资金设计尚未完整定义 `deal.reversed.v1` 和 `deposit.chargeback.v1`。Partner Service 上线前必须由权威生产方补齐：

- User 将现有 `UserApprovedEvent` 统一为版本化 `user.approved.v1`，Account 通过 Outbox 发布 `account.opened.v1/account.activated.v1`；事件至少包含 `entity_id`、`user_id`、`account_id`、`account_group_id`、事件版本和幂等业务键；
- Trading Outbox：`deal.created.v1/deal.reversed.v1`，包含 `account_group_id`、`spec_snapshot_id`、`contract_size`、`commission_amount`、`commission_currency`、`reversal_of_deal_id`；
- `customer_commission_received_amount` 在事件中统一为非负数；现有 `t_deal.commission` 的“负值表示扣费”只在交易域内部使用，事件适配层取其绝对值；
- Fund Chargeback 实体：关联原 `deposit_id`，记录拒付金额、币种、原因、时间和状态，并发布 completed/chargeback 事件；
- 所有生产方使用事务 Outbox，禁止依赖“写库成功后尽力发送 MQ”。

未具备 User/Account 契约时，不得宣称 ACCOUNT 归因物化或开户 CPA 可验收；未具备 Trading/Fund 契约时，只允许使用可验证的 `deal.created` 单级计提，不得上线成交冲正、首存 CPA 或拒付回拨。
...
| 角色 | 职责 |
|---|---|
| Broker Operator | 创建 Program、审核 Partner、发布规则 |
| Compliance Officer | KYB/AML、辖区与营销材料审查 |
| Partner Manager | 维护协议与 Partner 关系，不得审批自己的调账 |
| Settlement Maker | 生成结算批次 |
| Settlement Checker | 审批结算，不能与 Maker 为同一人 |
| Finance Payout Approver | 审批出款 |
| Partner Owner | 查看自身及授权下级数据、发起出款 |
| Partner Staff | 按 Partner 内部角色查看客户或报表 |
| Global Auditor | 跨系统只读审计，不得修改业务数据 |
...
同一主体同一时刻只能有一个有效归因。未来归因可以预先创建，但在生效时才原子切换 Current Pointer。Cookie 只是证据，不是权威关系；推荐码先解析为 `campaign_id + partner_id`，再写 Attribution。
...
distributable_revenue
  = customer_commission_received
  + realized_markup_revenue
  - lp_execution_cost
...
Partner Service 不自建 PSP 通道。它向 Fund Service 发起：

```json
{
  "entityId": "entity_id",
  "sourceType": "PARTNER_WALLET",
  "sourceId": "partner_id",
  "partnerId": "partner_id",
  "amount": "1000.00",
  "currency": "USD",
  "paymentMethodId": "xxx",
  "idempotencyKey": "partner_payout_no"
}
```

这要求 Fund Service 对现有 `t_withdrawal_order(account_id NOT NULL)` 增加 `entity_id + fund_source_type + fund_source_id` 通用资金来源契约，并令交易账户来源与 Partner 来源互斥，不能伪造交易账户 ID。Fund 必须校验请求实体与 Partner、Payment Method、Source 一致，幂等范围为 `(entity_id, sourceType, idempotencyKey)`。`fund_order_id` 专指 Fund 内部 BIGINT 主键，`fund_order_no` 专指 VARCHAR 业务单号，PSP reference 另存，不得混用。该改造完成前，Phase 1 外部出款不具备上线验收条件。

出款 Saga：

1. Partner Service 校验 Available、KYC/KYB、最低金额、支付方式和风险；
2. `PAYOUT_RESERVE` Journal：Available → Payout Clearing；
3. 创建 `t_partner_payout` 并调用 Fund；
4. Fund 成功：`PAYOUT_COMPLETE` Journal 将 Clearing → Cash/External Settlement，Payout=COMPLETED；
5. Fund 失败：`PAYOUT_RELEASE` Journal 将 Clearing → Available，Payout=FAILED；
6. 超时保持 PROCESSING，由对账任务查询，禁止直接重复出款。
...
- Account/Fund 通过 `t_account_ledger(TRANSFER_IN)` 入交易账户；
...
| 权限点 | 数据范围 | SoD |
|---|---|---|
| `partner.program.read/write/publish` | ENTITY | 发布需双人审批 |
| `partner.profile.review` | ENTITY | 不得审核本人创建的 Partner |
| `partner.hierarchy.transfer` | ENTITY | 必须独立审批 |
| `partner.agreement.write/publish` | ENTITY | 发布后不可改 |
| `partner.attribution.transfer` | ENTITY | 必须有原因与证据 |
| `partner.commission.recalculate` | ENTITY | 只生成差额 |
| `partner.adjustment.create/approve` | ENTITY | Maker/Checker |
| `partner.settlement.create/approve` | ENTITY | Maker/Checker |
| `partner.payout.approve` | ENTITY | 与 Settlement Checker 分离 |
| `partner.audit.read` | ENTITY/ALL | 只读 |

Partner Portal 仅允许 `SELF` 与授权的 `DESCENDANTS` 范围，并应用列级脱敏。

现有 PMS 的 `ALL/ORG_TREE/TEAM/SELF/CUSTOM` 不直接表达 Partner 子树：券商员工后台仍由 PMS 按 ENTITY/CUSTOM 授权；Partner Portal 的 `DESCENDANTS` 由 Partner Service 使用当前 `hierarchy_version + closure` 做业务行级鉴权，不把代理树伪装成 PMS 组织树。若 PMS 需要统一返回该范围，应新增 `PARTNER_NETWORK` scope provider，而不是复制层级数据。
...
CREATE TABLE `t_commission_adjustment` (
  ...
  `adjustment_type` VARCHAR(24) NOT NULL COMMENT 'REVERSAL/CLAWBACK/TIER_TRUE_UP/MANUAL/CORRECTION',
...
CREATE TABLE `t_partner_payout` (
  ...
  `fund_order_id` BIGINT UNSIGNED DEFAULT NULL,
  `fund_order_no` VARCHAR(64) DEFAULT NULL,
...
- USER/ACCOUNT 唯一归因；
- Deal 事件幂等；上游补齐 `deal.reversed.v1` 后启用冲正；
- 月结、单币种、人工审批；Fund 完成通用资金来源改造后启用外部出款；
- 基础报表、通知与对账。
...
1. `t_ib_client_relation`、`t_user.introducing_broker_id` 和账户推导不再同时充当权威；改为版本化 Attribution 单一真相。
2. 撮合引擎不再写佣金计提，消除“引擎写一次、Partner 消费事件再写一次”的重复计提风险。
3. 修复三级以上差额算法，使用相邻累计权益差，不递归扣减下级实得。
4. 不再把所有返佣简单描述为客户手续费的一部分；交易返佣和 CPA 分别受收入池与营销预算池约束。
5. 原 `wallet_balance` 不再是财务真相，升级为可重建的双分录 Partner Ledger。
6. 结算与出款解耦；Fund Service 必须支持 `PARTNER_WALLET`，不再伪用交易账户出金。
7. 增加 `entity_id`、版本、生效期、Inbox/Outbox、冲正、补算、税费、保留金、FX、SoD 和对账。
8. 品种范围使用 `symbol_category_id/symbol_group_id/symbol_id`，不再使用易漂移的文本类别。
9. 移除静态年份分区；首期保留全局幂等唯一键，达到阈值后按明确的分库/归档或 Idempotency Registry 方案演进。
10. 补齐 Partner Portal、Admin API、事件契约、权限、通知、风控、报表、上线阶段和验收标准。

本设计的核心不是“算出一笔返佣”，而是确保任何一笔返佣在多年后仍能回答：谁在什么规则下、基于哪个客户归因和业务事实、为何获得多少金额、由谁批准、如何入账、是否已出款以及后来是否发生冲正。
...
- **错误码**：遵循补充卷 §三，使用 `IB_*` 前缀。
- **事件**：遵循补充卷 §四 CloudEvents envelope。消费 `deal.created.v1`（见补充卷 §4.4 字段级 schema）、`revenue.realized.v1`（路由网关产，见补充卷 §4.4）、`deal.reversed.v1`、`deposit.chargeback.v1`。
- **角色**：遵循补充卷 §五，Partner Portal 权限由本域自定义 `PARTNER_NETWORK` scope provider，不直接用 PMS 角色。
- **ID**：遵循补充卷 §六，`partner_id`/`attribution_id` 由本域生成。
- **API**：OpenAPI 草案见 §16.3。
- **容灾/容量/监控**：遵循补充卷 §十/§十一/§十二。

### 16.2 `deal.created.v1` 字段级 Payload（引用补充卷 §4.4）
...
- `commission_amount` 为客户实收佣金（交易域权威）。
- `realized_markup_revenue` **不在 deal 事件**，由路由网关经 `revenue.realized.v1` 单独产出。
...
### 16.4 Fund Saga 超时状态机（补缺）

```
Payout 发起 → Fund PayoutClearing
  → PROCESSING（等待 Fund 回调）
  → 超时 24h 无响应：
      1. 状态 → RECONCILE
      2. 告警 P1
      3. 人工核对 Fund 是否实际出款
      4. Fund 确认出款 → COMPLETE
      5. Fund 确认未出款 → RELEASE（释放锁定金额）
  → Fund 回调成功 → COMPLETE
  → Fund 回调失败 → RELEASE
```

- `t_partner_payout.status` 增 `RECONCILE` 子状态。
- 监控：`payout_stuck_in_processing` > 0 告警。

### 16.5 跟单 Follower Deal 计提规则（补缺）

旧稿未覆盖跟单场景。v2.1 补：

- Follower Deal `source=SIGNAL`，`magic_number=subscription.magic`。
- **是否计入 IB 合格量**：由 `t_commission_plan.follower_eligible` 字段控制（默认 true）。
- **归因穿透**：Follower 账户的 `introducing_broker_id` 为权威，不穿透到 Master 的 IB。
- **Master IB 与 Follower IB**：若不同，各自计提，互不影响。

### 16.6 税务预扣（补缺）

旧稿 `tax_profile_json` 存在但无规则。v2.1 补：

- `t_partner_tax_profile`（新增）：`partner_id`、`jurisdiction`、`tax_type`（WHT/VAT）、`rate`、`threshold`。
- 计提时按 jurisdiction 预扣预提所得税（WHT），写入 `t_commission_accrual.tax_amount`。
- 结算时净额 = 计提 - 税。
- 报表：年度税务申报表，供 Partner 报税。

### 16.7 Reporting CQRS 读模型（补缺）

旧稿 Reporting Query 模块仅列名。v2.1 补：

- 写模型：`t_commission_accrual`/`t_partner_journal` 等（OLTP）。
- 读模型：物化视图 `mv_partner_daily_summary`（按 partner_id × day 聚合），刷新频率 5min。
- 大数据量报表：ClickHouse 投影，T+1 批处理。
- Partner Portal 查询走读模型，避免压 OLTP。
...
- 营销域**不得**直接改写 Attribution（补充卷 §五越界禁令）。

### 16.9 Phase 1 阻断项清单（补缺）

| 阻断项 | 状态 | 负责域 |
|--------|------|--------|
| Fund 支持 `PARTNER_WALLET` | 待资金域交付 | 资金 |
| `deal.reversed.v1` 事件生产 | 待交易域交付 | 交易 |
| `revenue.realized.v1` 事件生产 | 待路由网关交付 | 路由 |
| `deposit.chargeback.v1` 事件生产 | 待资金域交付 | 资金 |
| `user.approved.v1` 含 `introducing_broker_id` | 待准入域交付 | 准入 |
...
| `IB_NOTFOUND_ATTRIBUTION` | 无归因记录 | false |
...
|------|------|------|
| v2.0 | 2026-07 前 | 终版（旧） |
| v2.1 | 2026-07-21 | 对齐补充卷：deal.created.v1 字段级 schema、OpenAPI 草案、Fund Saga 超时状态机、跟单计提规则、税务预扣、Reporting CQRS、营销归因冲突升级、Phase1 阻断项、错误码 |
